...
class Dataset(object):
    ...
    def __init__(self, data_dir, embeddings, user_ctor, movie_ctor, response_ctor):
        ...
        self._response_ctor = response_ctor
        self._responses = self._read_responses(os.path.join(data_dir, "ratings.csv"))
    ...
    def _read_movies(self, path):
        ...
        for _, row in movie_df.iterrows():
            genres = [
                GENRE_MAP.get(genre, OTHER_GENRE_IDX) for genre in row.genres.split("|")
            ]
    ...
    def _populate_embeddings(self, initial_embeddings):
        ...
        for movie_ in self.get_movies():
            ...
            assert len(movie_.movie_vec) > len(
                GENRES
    ...
    def get_responses(self):
        """Returns an iterator over responses."""
