...
from absl.testing.absltest import TEST_SRCDIR
import attr
import file_util
import test_util
...
class MovieLensTest(absltest.TestCase):
    ...
    def setUp(self):
        ...
        self.data_dir = os.path.join(
            FLAGS.test_srcdir, os.path.split(os.path.abspath(__file__))[0], "test_data"
        )
    ...
    def test_observations_in_observation_space(self):
        for slate in [[0], [1], [2]]:
            ...
            for field in ["doc", "response", "user"]:
                self.assertIn(
                    observation[field], self.env.observation_space.spaces[field]
                )

    def test_user_can_score_document(self):
        user = self.user_sampler.get_user(1)
        for doc in self.dataset.get_movies():
            self.assertBetween(user.score_document(doc), user.MIN_SCORE, user.MAX_SCORE)
    ...
    def test_if_user_state_resets(self):
        ...
        ta_vec = np.copy(
            self.env._environment.user_model._user_sampler._users[
                curr_user_id
            ].topic_affinity
        )
        for i in range(3):
