...
def reindex(dataframes):
    ...
    index_dict = pd.Series(
        np.arange(movies.shape[0]), index=movies["movieId"].values
    ).to_dict()
...
def merge_with_genome_data(url, target_tag, dataframes):
    ...
    movies_df = movies_df.merge(tag_relevance_df, on="movieId", how="left").fillna(0)
    movies_df.rename(
...
def main(argv):
    ...
    dataframes = merge_with_genome_data(FLAGS.tag_genome_url, "violence", dataframes)
