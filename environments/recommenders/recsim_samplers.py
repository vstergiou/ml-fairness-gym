...
class UserPoolSampler(user.AbstractUserSampler):
    ...
    def __init__(
        self, users, user_ctor, seed=None, partitions=None, partition_seed=100
    ):
        ...
        Args:
          ...
          partition_seed: Used to control how users are randomly allocated to
            partitions.
        """
        super(UserPoolSampler, self).__init__(seed=seed, user_ctor=user_ctor)
    ...
    def set_active_pool(self, pool):
        if pool > len(self._partitions):
            raise ValueError(
                "Trying to select pool %d but there are only %d pools."
