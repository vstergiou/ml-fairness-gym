...
class FairnessEnv(gym.Env):
    ...
    assert not group_membership_var or (group_membership_var in observable_state_vars)

    def __init__(self, params=None, initialize_observation_space=True):
    ...
    def serialize_history(self):
        ...
        sanitized_history = [
            (json.loads(history_item.state.to_json()), history_item.action)
...
class Metric(object):
    ...
    def __init__(self, environment, realign_fn=None):
        ...
        self._environment_setter = (
            self._environment._set_state
    ...
    def _validate_history(self, history):
        ...
        for idx, (step, next_step) in enumerate(more_itertools.pairwise(history)):
            ...
            if simulated_state != next_step.state:
                raise ValueError(
                    "Invalid history at step %d %s != %s" % (idx, step, next_step)
                )
...
class Agent(object):
    ...
    def __init__(self, action_space, reward_fn, observation_space):
